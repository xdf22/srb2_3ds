...
consvar_t cv_grfogdensity = {"gr_fogdensity", "80", CV_SAVE|CV_CALL|CV_NOINIT, CV_Unsigned,
                           ...
                           FBITFIELD PolyFlags, INT32 lightlevel, lumpnum_t lumpnum, sector_t *FOFsector, UINT8 alpha, boolean fogplane, extracolormap_t *planecolormap)
{
    ...
    extern void NDS3D_driverLog(const char *s, ...);
    /*
    static size_t dupCntr;
    static void *prev;
    if(prev == pv)
    {
        dupCntr++;
        NDS3D_driverLog("0x%X\n", (unsigned)dupCntr);
    }
    else prev = pv;
    */

    //NDS3D_driverLog("0x%X 0x%X\n", (unsigned)pv, height);
	...
	// Allocate plane-vertex buffer
	planeVerts = HWR_AllocVertexBuffer(nrPlaneVerts);
...
static void HWR_RenderSkyPlane(extrasubsector_t *xsub, fixed_t fixedheight)
{
	...
	// transform
	v3d = HWR_AllocVertexBuffer(nrPlaneVerts);
...
static void HWR_DrawSegsSplats(FSurfaceInfo * pSurf)
{
	...
	for (; splat; splat = splat->next)
	{
		...
		// transform
        trVerts = HWR_AllocVertexBuffer(4);
		wv = trVerts;
...
{
	FOutVector  *trVerts = HWR_AllocVertexBuffer(4);
	FOutVector  *wv;
...
{
	FOutVector *swallVerts = HWR_AllocVertexBuffer(4);
	FSurfaceInfo sSurf;
...
static void HWR_SplitSprite(gr_vissprite_t *spr)
{
	...
	for (i = 0; i < sector->numlights; i++)
	{
        ...
        FOutVector *wv = HWR_AllocVertexBuffer(4);
		...
		wallVerts[0].y = wallVerts[1].y = bot;
#endif
        memcpy(wv, wallVerts, sizeof wallVerts);
		...
		HWD.pfnDrawPolygon(&Surf, wv, 4, blend|PF_Modulated|PF_Clip);
...
static void HWR_DrawSprite(gr_vissprite_t *spr)
{
    ...
    wallVerts = HWR_AllocVertexBuffer(4);
...
static void HWR_CreateDrawNodes(void)
{
	...
	for (i = 0; i < numpolyplanes; i++)
    {
        polyplaneinfo_t *polyplane = &polyplaneinfo[i];
    ...
    NDS3D_ResetRenderStatsMeasureEnd(6);
...
{
    NDS3D_ResetRenderStatsMeasureBegin(0);

    NDS3D_ResetRenderStatsMeasureBegin(2);
    ...
    NDS3D_ResetRenderStatsMeasureEnd(2);

    NDS3D_ResetRenderStatsMeasureBegin(1);
	HWR_RenderBSPNode((INT32)numnodes-1);
    NDS3D_ResetRenderStatsMeasureEnd(1);

    NDS3D_ResetRenderStatsMeasureBegin(3);
	...
	// Draw MD2 and sprites
    NDS3D_ResetRenderStatsMeasureBegin(10);
#ifdef SORTING
	HWR_SortVisSprites();
#endif
    NDS3D_ResetRenderStatsMeasureEnd(10);
    NDS3D_ResetRenderStatsMeasureBegin(11);
#ifdef SORTING
	HWR_DrawSprites();
#endif
    NDS3D_ResetRenderStatsMeasureEnd(11);
	...
	if (numplanes || numpolyplanes || numwalls) //Hurdler: render 3D water and transparent walls after everything
	{
		NDS3D_ResetRenderStatsMeasureBegin(5);
        HWR_CreateDrawNodes();
        NDS3D_ResetRenderStatsMeasureEnd(5);
	}
#else
    ...
    NDS3D_ResetRenderStatsMeasureEnd(3);

    NDS3D_ResetRenderStatsMeasureEnd(0);
}
