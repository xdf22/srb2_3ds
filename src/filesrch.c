...
consvar_t cv_addons_option = {"addons_option", "1", CV_SAVE|CV_CALL, addons_cons_t, Addons_option_Onchange, 0, NULL, NULL, 0, 0, NULL};
...
consvar_t cv_addons_showall = {"addons_showall", "Yes", CV_SAVE, CV_YesNo, NULL, 0, NULL, NULL, 0, 0, NULL};
...
void searchfilemenu(char *tempname)
{
	...
	for (i = first; i < sizecoredirmenu; i++)
	{
		if (filemenucmp(coredirmenu[i]+DIR_STRING, localmenusearch))
		{
			...
			dirmenu[sizedirmenu++] = coredirmenu[i];
			CONS_Printf("ADD: %s\n", coredirmenu[i] + DIR_STRING);
		}
	}
...
boolean preparefilemenu(boolean samedepth)
{
	...
	if (!sizecoredirmenu)
	{
		CONS_Printf("Addons menu: no valid entries found in %s\n", menupath);
		dir_on[menudepthleft] = 0;
		closefilemenu(false);
		return false;
	}
